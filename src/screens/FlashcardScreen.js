...
export default function FlashcardScreen() {
  ...
  const loadCards = async () => {
    try {
      const words = await getWordsForReview(user.uid);
  ...
  const handleKnownWord = async () => {
    try {
      if (cards.length > 0 && currentIndex < cards.length) {
        ...
        await updateWordAfterReview(user.uid, currentWord.id, true);
  ...
  const handleNeedsPractice = async () => {
    try {
      if (cards.length > 0 && currentIndex < cards.length) {
        const currentWord = cards[currentIndex];
        await updateWordAfterReview(user.uid, currentWord.id, false);
